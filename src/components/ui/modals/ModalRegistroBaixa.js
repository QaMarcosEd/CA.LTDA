...
export default function ModalVenda({ isOpen, onClose, produto, onSubmit }) {
  ...
  useEffect(() => {
    if (produto && quantidade) {
      ...
      if (!isNaN(qty) && qty > 0) {
        ...
        // Recalcula taxa e líquido quando valorTotal muda
  ...
  }, [quantidade, produto, formaPagamento, bandeira, modalidade, taxasCartao]);
  ...
  const handleConfirm = async () => {
    ...
    if (isParcelado) {
      if (formaPagamento !== 'PROMISSORIA' && !(formaPagamento === 'CARTAO' && ['2_3X', '4_6X'].includes(modalidade))) {
        toast.error('Parcelamento só é permitido com Promissória ou Cartão (2-3x ou 4-6x)');
        return;
      }
      const entradaValor = parseFloat(entrada) || 0;
      if (entradaValor > parseFloat(valorTotal)) {
    ...
    } else if (formaPagamento === 'PROMISSORIA' || (formaPagamento === 'CARTAO' && ['2_3X', '4_6X'].includes(modalidade))) {
        ...
        <div className="mb-4">
          ...
          {isParcelado && (
            <div className="mt-2 space-y-4">
              ...
              <p className="text-sm font-poppins text-gray-600">
                Valor por parcela: R$ {calcularValorParcela()}
                {formaPagamento === 'CARTAO' && modalidade && ['2_3X', '4_6X'].includes(modalidade) && (
                  <> | Líquido por parcela: R$ {(calcularValorParcela() * (1 - taxasCartao.find(t => t.bandeira === bandeira && t.modalidade === modalidade)?.taxaPercentual / 100)).toFixed(2)}</>
                )}
              </p>
            </div>
          )}
