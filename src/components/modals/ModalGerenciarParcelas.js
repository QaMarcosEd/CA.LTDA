...
/* MUDEI AS 11:23 QUALQUER COISA VC VOLTA AO ANTERIOR*/
...
export default function ModalGerenciarParcelas({ isOpen, venda, onClose, marcarParcelaComoPaga }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      venda?.parcelas.forEach((parcela) => {
        ...
        if (isCartao) {
          const formaParts = parcela.formaPagamento.split('_');
          const bandeira = formaParts[1] || '';
          const modalidade = formaParts[2] || '';
          const valor = parcela.valorLiquido - (parcela.valorPago || 0);
          let taxa = 0;
          let valorLiquido = valor;
          ...
          initialFormasPagamento[parcela.id] = {
            forma: 'CARTAO',
            bandeira,
            modalidade,
            taxa,
      ...
      setFormasPagamento(initialFormasPagamento);
    } else {
      setFormasPagamento({});
    }
  ...
  const handleFormaPagamentoChange = (parcelaId, forma, bandeira, modalidade, valorPago) => {
    setFormasPagamento((prev) => {
      ...
      newState[parcelaId] = {
        forma: forma || prev[parcelaId]?.forma || 'PROMISSORIA',
        bandeira: bandeira || prev[parcelaId]?.bandeira || '',
        modalidade: modalidade || prev[parcelaId]?.modalidade || '',
        taxa,
        valorLiquido,
      };
      return newState;
                ...
                {!parcela.pago && (
                  <div className="mt-2 space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            handleFormaPagamentoChange(
                              parcela.id,
                              isCartao ? 'CARTAO' : formasPagamento[parcela.id]?.forma || 'PROMISSORIA',
                              isCartao ? bandeira : formasPagamento[parcela.id]?.bandeira || '',
                              isCartao ? modalidade : formasPagamento[parcela.id]?.modalidade || '',
                              e.target.value
                            );
                          }}
                        />
                      ...
                      {!isCartao && formasPagamento[parcela.id]?.forma === 'CARTAO' && (
                        <>
                          <div>
                            <select
                              className="border border-gray-200 p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 font-poppins text-xs sm:text-sm w-full transition-all"
