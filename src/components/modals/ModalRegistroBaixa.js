...
//   useEffect(() => {
//     if (produto) {
//       // Inicializa valorTotal com base no preço unitário se quantidade não for definida
//       setValorTotal(produto.precoVenda.toFixed(2));
//     }
//     if (isOpen) {
//       fetch('/api/clientes', { cache: 'no-store' })
...
//   useEffect(() => {
//     // Atualiza valorTotal apenas se quantidade for alterada e valorTotal ainda não foi editado manualmente
//     if (produto && quantidade && !isNaN(parseInt(quantidade, 10)) && parseInt(quantidade, 10) > 0) {
//       const novoValorTotal = (produto.precoVenda * parseInt(quantidade, 10)).toFixed(2);
//       if (!valorTotal || parseFloat(valorTotal) === produto.precoVenda) { // Só atualiza se não foi editado
//         setValorTotal(novoValorTotal);
//       }
//     }

//     // Recalcula taxa e valor líquido com base no valorTotal atual
//     if (formaPagamento === 'CARTAO' && bandeira && modalidade && valorTotal) {
//       const taxaCartao = taxasCartao.find(
//         (t) => t.bandeira === bandeira && t.modalidade === modalidade
//       );
//       if (taxaCartao) {
...
//       } else {
//         setTaxa(0);
//         setValorLiquido(parseFloat(valorTotal).toFixed(2));
//       }
//     } else {
//       setTaxa(0);
//       setValorLiquido(parseFloat(valorTotal).toFixed(2));
...
//           {/* Valor Total - Agora editável sem sobrescrita */}
//           <div className="relative">
//             <input
//               type="number"
//               step="0.01"
...
//             />
//             <label className="absolute left-4 -top-2 text-xs bg-gray-50 px-1 text-gray-500 pointer-events-none transition-all duration-200 peer-focus:text-green-500">
//               Valor Total (R$) <span className="text-red-500 ml-1">*</span> (inclui desconto)
//             </label>
//           </div>
...
export default function ModalRegistroBaixa({ isOpen, onClose, produto, onSubmit, onSubmitRefresh }) {
  ...
  useEffect(() => {
    if (produto && quantidade && !isNaN(parseInt(quantidade, 10)) && parseInt(quantidade, 10) > 0) {
      const novoValorTotal = (produto.precoVenda * parseInt(quantidade, 10)).toFixed(2);
      if (!valorTotal || parseFloat(valorTotal) === produto.precoVenda) {
        setValorTotal(novoValorTotal);
      }
    }
    if (formaPagamento === 'CARTAO' && bandeira && modalidade && valorTotal) {
  ...
  const handleConfirm = async () => {
    if (isSubmitting) return; // Impede múltiplos cliques enquanto a submissão está em andamento
    setIsSubmitting(true); // Marca como em processamento
    ...
    try {
      ...
      setIsSubmitting(false); // Libera o botão após a conclusão (sucesso ou erro)
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full m-4 flex flex-col max-h-[90vh] overflow-hidden transform transition-all duration-300">
        ...
        <div className="p-6 bg-white border-t border-gray-200 flex gap-4">
          ...
          <button
            ...
            className={`flex-1 px-6 py-3 rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2 shadow-md text-sm md:text-base font-poppins
              ${isSubmitting ? 'bg-gray-400 text-gray-700 cursor-not-allowed' : 'bg-green-500 text-white hover:bg-green-600'}`}
          >
