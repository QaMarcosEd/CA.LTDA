...
export async function getDashboardData(epoca = 'normal') {
  try {
    ...
    const resumoVendas = vendas.reduce(
      (acc, v) => {
        if (v.status === 'QUITADO') acc.totalQuitado += v.valorTotal;
        else acc.totalPendente += v.valorTotal;
        return acc;
      },
      { totalQuitado: 0, totalPendente: 0 }
    );

    // 3. RANKING DE MODELOS VENDIDOS
    const vendasComProduto = await prisma.venda.findMany({
      select: { quantidade: true, produto: { select: { modelo: true } } },
    });

    const rankingMap = vendasComProduto.reduce((acc, v) => {
      const modeloRaw = v.produto.modelo || 'Sem Modelo';
      const chave = normalizar(modeloRaw);
      if (!acc[chave]) acc[chave] = { modelo: modeloRaw.trim(), qtyVendida: 0 };
      acc[chave].qtyVendida += v.quantidade;
      return acc;
    }, {});

    const rankingVendidos = Object.values(rankingMap)
