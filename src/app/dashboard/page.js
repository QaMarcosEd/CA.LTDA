...
//   // Busca listas dinâmicas para selects (gêneros e modelos únicos)
...
//           </div>
//           <div className="bg-white p-4 rounded-lg shadow-md">
//             <h3 className="text-sm font-poppins text-gray-600">Valor Estoque</h3>
//             <p className="text-2xl font-bold font-poppins text-green-600">R$ {valorTotalEstoque}</p>
//           </div>
//           <div className="bg-white p-4 rounded-lg shadow-md">
//             <h3 className="text-sm font-poppins text-gray-600">Vendas Quitadas</h3>
//             <p className="text-2xl font-bold font-poppins text-purple-600">R$ 0.00</p> {/* Ajustar com backend */}
//           </div>
//           <div className="bg-white p-4 rounded-lg shadow-md">
//             <h3 className="text-sm font-poppins text-gray-600">Vendas Pendentes</h3>
//             <p className="text-2xl font-bold font-poppins text-red-600">R$ 0.00</p> {/* Ajustar com backend */}
//           </div>
//         </div>
...
//                     datasets: [{
//                       label: 'Estoque por Gênero',
//                       data: contagemPorGeneroAgrupado.map((item) => item.total) || [0],
//                       backgroundColor: colors,
//                       borderColor: borderColors,
//                       borderWidth: 1,
//                     }],
...
//                 <Pie
//                   data={{
//                     labels: contagemPorModeloAgrupado.map((item) => item.modelo) || ['Sem dados'],
//                     datasets: [{
//                       label: 'Estoque por Modelo',
//                       data: contagemPorModeloAgrupado.map((item) => item.total) || [0],
//                       backgroundColor: colors,
//                       borderColor: borderColors,
//                       borderWidth: 1,
//                     }],
//                   }}
//                   options={{ ...chartOptions, plugins: { ...chartOptions.plugins, title: { text: 'Estoque por Modelo' } } }}
//                 />
//               </div>
//             </div>
//             <div className="mt-4 max-h-32 overflow-y-auto">
//               <div className="grid grid-cols-2 gap-2 pr-1">
//                 {contagemPorModeloAgrupado.map((item, i) => (
//                   <div key={i} className="flex items-center text-xs font-poppins text-gray-700">
//                     <span
//                       className="w-2 h-2 rounded-full mr-1"
//                       style={{ backgroundColor: colors[i % colors.length] }}
...
//                 <Pie
//                   data={{
//                     labels: contagemPorMarcaAgrupado.map((item) => item.marca) || ['Sem dados'],
//                     datasets: [{
//                       label: 'Estoque por Marca',
//                       data: contagemPorMarcaAgrupado.map((item) => item.total) || [0],
//                       backgroundColor: colors,
//                       borderColor: borderColors,
//                       borderWidth: 1,
//                     }],
//                   }}
//                   options={{ ...chartOptions, plugins: { ...chartOptions.plugins, title: { text: 'Estoque por Marca' } } }}
//                 />
//               </div>
//             </div>
//             <div className="mt-4 max-h-32 overflow-y-auto">
//               <div className="grid grid-cols-2 gap-2 pr-1">
//                 {contagemPorMarcaAgrupado.map((item, i) => (
//                   <div key={i} className="flex items-center text-xs font-poppins text-gray-700">
//                     <span
//                       className="w-2 h-2 rounded-full mr-1"
//                       style={{ backgroundColor: colors[i % colors.length] }}
...
export default function Dashboard() {
  ...
  // Busca listas dinâmicas para selects
  const fetchListasFiltros = async () => {
    try {
      const [generosRes, modelosRes] = await Promise.all([
        fetch('/api/produtos?tipo=genero'),
  ...
  const fetchProdutos = async (pg = 1) => {
    ...
    try {
      ...
      const data = await res.json();
      setProdutos(data.data || []);
      setTotalPages(Math.ceil(data.totalProdutos / limit) || 1); // Ajustado pra totalProdutos
      setTotalProdutos(data.totalProdutos || 0);
      setValorTotalEstoque(data.valorTotalEstoque || 0);
    } catch (err) {
      console.error('Erro no fetchProdutos:', err);
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white p-4 rounded-lg shadow-md">
            <h3 className="text-sm font-poppins text-gray-600">Vendas Quitadas</h3>
            <p className="text-2xl font-bold font-poppins text-purple-600">R$ {resumoVendas.totalQuitado}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-md">
            <h3 className="text-sm font-poppins text-gray-600">Vendas Pendentes</h3>
            <p className="text-2xl font-bold font-poppins text-red-600">R$ {resumoVendas.totalPendente}</p>
          </div>
        </div>
