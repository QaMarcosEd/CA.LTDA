...
//           {[
//             { icon: Package, label: 'Valor Estoque', value: `R$ ${produtos.reduce((sum, p) => sum + ((p.quantidade || 0) * (p.precoVenda || 0)), 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, color: '#394189' },
//             { icon: Zap, label: 'Pares Totais', value: produtos.reduce((sum, p) => sum + (p.quantidade || 0), 0).toLocaleString('pt-BR'), color: '#10B981' },
//             { icon: TrendingUp, label: 'Esgotados', value: produtos.filter(p => p.quantidade === 0).length.toLocaleString('pt-BR'), color: '#F59E0B' },
...
export default function Estoque() {
  ...
  const handleSubmitLote = async (data) => {
    const response = await fetch('/api/estoque/lote', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    });
